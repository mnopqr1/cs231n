...
def svm_loss_vectorized(W, X, y, reg):
  ...
  A = X @ W
  Y = torch.ones_like(A)
  Y[torch.arange(num_train), y] = 0
  SC = A[torch.arange(num_train), y]
  RL = A - torch.reshape(SC, (num_train, 1)) + Y
  L = torch.maximum(RL, torch.zeros_like(RL))
  loss = torch.sum(L) / num_train + reg * torch.sum(W * W)
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  ...
  #############################################################################
  # Replace "pass" statement with your code
  wrongs = torch.gt(RL, torch.zeros_like(RL)).to(torch.float64)
  dWwrong = torch.transpose(X, 0, 1) @ wrongs
  nwrong = torch.reshape(torch.sum(torch.gt(L, 0).to(torch.float64), 1), (num_train, 1))
  scaledX = -nwrong * X
  dWright = torch.transpose(scaledX, 0,1) @ (1 - Y)
...
def svm_get_search_params():
  ...
  #regularization_strengths=[1e1]
  learning_rates = [1e-1, 1e-2, 1e-3]#, 1e-4]
  regularization_strengths = [0.1, 0.5, 1e0]#, 1e1]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
...
def test_one_param_set(cls, data_dict, lr, reg, num_iters=2000):
  ...
  # Replace "pass" statement with your code
  loss_hist = cls.train(data_dict['X_train'], 
                        data_dict['y_train'], 
                        learning_rate=lr, 
                        reg=reg,
                        num_iters=num_iters)
  ...
  train_acc = torch.mean(torch.eq(
    data_dict['y_train'], 
    y_train_pred).to(torch.float64))
  val_acc = torch.mean(torch.eq(
    data_dict['y_val'], 
    y_val_pred
  ).to(torch.float64))
  ############################################################################
